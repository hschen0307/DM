...
nonClick_set = nonClick_all[sample(nrow(nonClick_all), 60000),] #same size as clicked 
...
#accuracy(AVE) 64%
rfmodel=randomForest(clicked ~ confidence_level+topic_id, data=train_set, ntree=3)
rfmodel.probs=predict(rfmodel,test_set)
rfmodel.pred=ifelse(rfmodel.probs>0.5,1,0)
table(rfmodel.pred, test_set$clicked)
rf_acc1 = mean(rfmodel.pred == test_set$clicked)     
 

rfmodel=randomForest(clicked ~ confidence_level+topic_id, data=train_set, ntree=30)
rfmodel.probs=predict(rfmodel,test_set)
rfmodel.pred=ifelse(rfmodel.probs>0.5,1,0)
table(rfmodel.pred, test_set$clicked)
rf_acc2 = mean(rfmodel.pred == test_set$clicked)     
...
rfmodel.pred=ifelse(rfmodel.probs>0.5,1,0)
table(rfmodel.pred, test_set$clicked)
rf_acc3 = mean(rfmodel.pred == test_set$clicked)     
...
lr_acc1=mean(lrmodel.pred==test_set$clicked)     
lr_acc1
...
lrmodel.pred=ifelse(lrmodel.probs >0.5,1,0)
table(lrmodel.pred,test_set$clicked)
lr_acc2=mean(lrmodel.pred==test_set$clicked)     
lr_acc2
...
lrmodel.pred=ifelse(lrmodel.probs >0.5,1,0)
table(lrmodel.pred,test_set$clicked)
lr_acc3=mean(lrmodel.pred==test_set$clicked)     
lr_acc3
...
lrmodel.pred=ifelse(lrmodel.probs >0.5,1,0)
table(lrmodel.pred,test_set$clicked)
lr_acc4=mean(lrmodel.pred==test_set$clicked)     
lr_acc4


#final result plot
allAcc <- c(rf_acc1, lr_acc1, lr_acc2,lr_acc3, lr_acc4)
barplot(allAcc, main="Accuracy Comparison", xlab="Models", ylab="Accuracy", names.arg=c("RadomForest","Logistic1","Logistic2","Logistic3","Logistic4"))
allAcc2 <- c(rf_acc1, rf_acc2, rf_acc3)
barplot(allAcc2, main="Accuracy Comparison(Random Forest)", xlab="number of trees", ylab="Accuracy", names.arg=c("3","30","300"))
